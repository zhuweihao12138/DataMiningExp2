...
int main(int argc, char** argv) {
    ...
    // create LightningGraph, cleaning (TODO)
    ...
    std::cout << bm.test_write_vertex(n) << std::endl;
    std::cout.flush();
    std::cout << bm.test_write_edge(n * 10) << std::endl;
    std::cout.flush();

    std::cout << bm.test_read_neighbour(n) << std::endl;
    std::cout.flush();
    std::cout << bm.test_shortest_path(n) << std::endl;
    std::cout.flush();

    std::cout << bm.test_read_neighbour_mt(n, num_threads) << std::endl;
    std::cout.flush();
    std::cout << bm.test_shortest_path_mt(n, num_threads) << std::endl;
    std::cout.flush();
