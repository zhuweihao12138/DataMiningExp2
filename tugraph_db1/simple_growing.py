...
def read_neighbour(no):
    ...
    while True:
        try:
            vid = 0
            it = txn.GetVertexIndexIterator("person", "no", Data.genBuf(no), Data.genBuf(no))
            if it.IsValid():
                vid = it.GetVid()
            else:
                ok = False
...
class BenchmarkLightningGraph:
    ...
    def __encode_no(self, x):
        s = str(x)
        while len(s) < self.__length:
            s = "0" + s
        return s

    def __random_no(self):
        return self.__encode_no(random.randint(0, self.__vertices - 1))

    def __random_name(self):
        return random_generator(self.__length)

    def __write_vertex(self, no, name):
        txn = self.__db.CreateWriteTxn(True)
        vid = txn.AddVertex("person", ["no", "name"], [Data.genBuf(no), Data.genBuf(name)])
        txn.Commit()
    ...
    def __write_edge(self, no_from, no_to):
        ...
        while True:
            try:
                it = txn.GetVertexIndexIterator("person", "no", Data.genBuf(no_from), Data.genBuf(no_from))
                if it.IsValid():
                    vid_from = it.GetVid()
                else:
                    ok = False
                it = txn.GetVertexIndexIterator("person", "no", Data.genBuf(no_to), Data.genBuf(no_to))
                if it.IsValid():
                    vid_to = it.GetVid()
                else:
                    ok = False
    ...
    def test_write_vertex(self, count):
        for i in range(count):
            no = self.__encode_no(self.__vertices)
...
if __name__ == "__main__":
    ...
    db.AddLabel("person",
                [FieldDef("no", DataType.LBR_STRING, 0, False), FieldDef("name", DataType.LBR_STRING, 0, False)], True)
