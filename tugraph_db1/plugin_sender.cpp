...
void plugin_thread_func(multithread_client::ClientThread* thread) {
    while (true) {
        ...
        bool ret = thread->get_channel()->CallPlugin(res, obj["PluginType"].get<std::string>(),
                obj["PluginName"].get<std::string>(),obj["Param"].dump(),
                obj["Timeout"].get<double>(),obj["InProcess"].get<bool>(),
...
namespace multithread_client {
    ...
    PluginSender::PluginSender(const Config& conf)
            : config(conf)
            , pool(conf) {}

    bool PluginSender::parse_line(std::string& line, uint32_t& times) {
        boost::trim_all(line);
        nlohmann::json obj = nlohmann::json::parse(line);
        if (!obj.contains("PluginName") || !obj.contains("PluginType") || !obj.contains("Timeout")
            || !obj.contains("InProcess") || !obj.contains("Graph") || !obj.contains("Times")
            || !obj.contains("Param")) {
            std::cout << "configuration file missing field" << std::endl;
            return false;
        }
    ...
    void PluginSender::process() {
        auto start = std::chrono::steady_clock::now();
        pool.start(std::bind(&plugin_thread_func, std::placeholders::_1));
        std::thread reader(&PluginSender::file_reader, this);
        reader.join();
        pool.stop();
        pool.join();
        auto end = std::chrono::steady_clock::now();
    ...
    void PluginSender::calculate(uint64_t time_used) {
