 ...
 */
package com.antgroup.tugraph.ogm.autoindex;
...
import com.antgroup.tugraph.ogm.session.request.RowDataStatement;
...
class AutoIndex {
    ...
    private static String createDescription(String name, IndexType type, String owningType, String[] properties) {
        ...
        if (name == null) {
            switch (type) {
                case SINGLE_INDEX:
                    validatePropertiesLength(properties, IndexType.SINGLE_INDEX);
                    return "INDEX ON :`" + owningType + "`(`" + properties[0] + "`)";

                case UNIQUE_CONSTRAINT:
                    validatePropertiesLength(properties, IndexType.UNIQUE_CONSTRAINT);
                ...
                case NODE_PROP_EXISTENCE_CONSTRAINT:
                    validatePropertiesLength(properties, IndexType.NODE_PROP_EXISTENCE_CONSTRAINT);
    ...
    static Optional<AutoIndex> parseIndex(Map<String, Object> indexRow) {
        ...
        if (indexRow.containsKey("properties") && indexRow.containsKey("labelsOrTypes") && indexRow.get("labelsOrTypes") instanceof String[]) {
            ...
            return Optional.of(new AutoIndex(null, indexProperties.length > 1 ? IndexType.COMPOSITE_INDEX : IndexType.SINGLE_INDEX,
    ...
    public AutoIndex createOppositeIndex() {
        switch (type) {
            case SINGLE_INDEX:
                return new AutoIndex(null, IndexType.UNIQUE_CONSTRAINT, owningType, properties, name);

            case UNIQUE_CONSTRAINT:
                return new AutoIndex(null, IndexType.SINGLE_INDEX, owningType, properties, name);

            case COMPOSITE_INDEX:
                return new AutoIndex(null, IndexType.NODE_KEY_CONSTRAINT, owningType, properties, name);

            case NODE_KEY_CONSTRAINT:
                return new AutoIndex(null, IndexType.COMPOSITE_INDEX, owningType, properties, name);
