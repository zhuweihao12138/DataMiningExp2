...
TEST_F(TestGraph, Graph) {
    ...
    std::shared_ptr<KvTable> meta_table =
        store->OpenTable(*txn, "meta", true, lgraph::ComparatorDesc::DefaultComparator());
    Graph graph(*txn, std::move(graph_table), meta_table);
    txn->Commit();
    ...
    UT_LOG() << "Testing iterator refresh";
    {
        ...
        for (size_t i = 0; i < nv; i++) {
            ...
            if (i >= nv / 2) {
                vits.push_back(graph.GetUnmanagedVertexIterator(txn.get(), i, false));
            }
        }
        VertexId vid = nv / 2;
        for (auto& it : vits) {
    ...
    // delete
    {
        ...
        {
            size_t nd = 0;
            graph::OutEdgeIterator oeit = graph.GetUnmanagedOutEdgeIterator(
                txn.get(), EdgeUid(), true);
            EdgeId eid = 0;
            VertexId dstid = 0;
            ...
            OutEdgeIterator oeit2 = graph.GetUnmanagedOutEdgeIterator(
                txn.get(), EdgeUid(nv - 1, nv - 1, 0, 0, 0), true);
...
TEST_F(TestGraph, DeleteVertexRemoveEdges) {
    ...
    txn = store->CreateWriteTxn();
    {
        ...
        UT_EXPECT_TRUE(
            !graph.GetUnmanagedInEdgeIterator(txn.get(), EdgeUid(0, 1, 0, 0, 0), true).IsValid());
        UT_EXPECT_TRUE(
            !graph.GetUnmanagedInEdgeIterator(txn.get(), EdgeUid(0, 2, 0, 0, 0), true).IsValid());
        auto vit = graph.GetUnmanagedVertexIterator(txn.get(), 1);
        auto eit = vit.GetInEdgeIterator();
        UT_EXPECT_FALSE(eit.IsValid());
        auto vit2 = graph.GetUnmanagedVertexIterator(txn.get(), 2);
