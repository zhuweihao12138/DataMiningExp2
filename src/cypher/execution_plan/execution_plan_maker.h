...
class ExecutionPlanMaker : public geax::frontend::AstNodeVisitor {
 public:
    ...
    const ResultInfo& GetResultInfo() const { return result_info_; }
 ...
 private:
    ...
    DISABLE_MOVE(ExecutionPlanMaker);

    std::string _DumpPlanBeforeConnect(int indent, bool statistics) const;
    static void _UpdateStreamRoot(OpBase* new_root, OpBase*& root);
    static OpBase* _SingleBranchConnect(const std::vector<OpBase*>& ops);
    void _AddScanOp(const SymbolTable* sym_tab, Node* node, std::vector<OpBase*>& ops,
                    bool skip_arg_op);
